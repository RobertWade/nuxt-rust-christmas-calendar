...
export const useCalendarStore = defineStore('calendar', () => {
   ...
   const doorList = computed<CalendarDoor[]>(() => calendar.value?.doors ?? [])
   const activeDoor = computed<CalendarDoor | null>(() => doorList.value.find(door => door.day === currentDoor.value) ?? null)
   ...
   function upsertDoor(door: CalendarDoor) {
      ...
      const index = calendar.value.doors.findIndex(existing => existing.day === door.day)
      if (index === -1) {
   ...
   return { calendar, doorList, activeDoor, currentDoor, setCalendar, setCurrentDoor, upsertDoor, clearCalendar }
})